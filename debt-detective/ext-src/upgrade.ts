...
export class Upgrade implements vscode.CodeActionProvider {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    const command = vscode.commands.registerCommand('debt-detective.upgrade', async (range: vscode.Range) => {
      ...
      vscode.tasks.executeTask(new vscode.Task({type: 'shell'},
  ...
  provideCodeActions(document: vscode.TextDocument, range: vscode.Range | vscode.Selection, context: vscode.CodeActionContext, token: vscode.CancellationToken): vscode.ProviderResult<(vscode.CodeAction | vscode.Command)[]> {
    return context.diagnostics
      .filter(diagnostic => diagnostic.source === 'debt-detective')
      .map(diagnostic => this.createCommandCodeAction(diagnostic));
  }

  private createCommandCodeAction(diagnostic: vscode.Diagnostic): vscode.CodeAction {
    const text = vscode.window.activeTextEditor?.document.getText(diagnostic.range);
    const action = new vscode.CodeAction(`Install ${text} package...`, vscode.CodeActionKind.QuickFix);
